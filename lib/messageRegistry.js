...
exports.MESSAGE_REGISTRY = void 0;
exports.getMessage = getMessage;
exports.formatMessage = formatMessage;
exports.packMessageToCell = packMessageToCell;
const core_1 = require("@ton/core");
// ─── Snake-data serialisation ─────────────────────────────────────────────────
const CELL_MAX_BYTES = 127;
function encodeSnakeData(text) {
    const textBytes = Buffer.from(text, "utf8");
    function packChunk(offset) {
        const chunk = textBytes.subarray(offset, offset + CELL_MAX_BYTES);
        const builder = (0, core_1.beginCell)().storeBuffer(chunk);
    ...
    const rootBuilder = (0, core_1.beginCell)()
...
function getMessage(key) {
    const entry = exports.MESSAGE_REGISTRY[key];
    if (!entry)
        throw new Error(`[Registry] Key ${key} not found`);
    return entry;
}
 ...
 * Unified version for bulkMint.ts
 */
function packMessageToCell(key, suffix) {
    const entry = getMessage(key);
 ...
 */
function serializeSnakeData(data) {
    const chunks = [];
    for (let i = 0; i < data.length; i += 127) {
        chunks.push(data.slice(i, i + 127));
    }
    let cur = (0, core_1.beginCell)();
    for (let i = chunks.length - 1; i >= 0; i--) {
        const chunk = chunks[i];
        const next = cur;
        cur = (0, core_1.beginCell)();
