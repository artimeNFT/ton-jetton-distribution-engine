...
export async function run(provider: NetworkProvider): Promise<void> {
    ...
    for (let i = 0; i < MINT_ADDRESSES.length; i++) {
        ...
        try {
            ...
            const recipientWallet = provider.open(JettonWallet.fromAddress(recipientWalletAddress));
            const recipientData = await recipientWallet.getGetWalletData();

            console.log(`  👛 Recipient wallet : ${recipientWalletAddress.toString()}`);
            console.log(`  💰 Recipient balance: ${Number(recipientData.balance) / 1_000_000} tokens`);
